...
| **agent/deploy/agent-framework-deploy** | Deploy AI agents and workflows built with Microsoft Agent Framework SDK to Microsoft Foundry. Handles Dockerfile generation, ACR container build, hosted agent version creation, and container startup. | [agent/deploy/agent-framework-deploy/SKILL.md](agent/deploy/agent-framework-deploy/SKILL.md) |
...
> 💡 **Model Deployment:** Use `models/deploy-model` for model deployment scenarios — it intelligently routes between quick preset deployment, customized deployment with full control, and capacity discovery across regions.